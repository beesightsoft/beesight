...
const changeAppIdSync = (projectPath, appName, newAppId, currentAppId) => {
    if (newAppId != currentAppId) {
        ...
        var paths = [
            ...
            projectPath + '/android/app/src/main/java/' + currentAppIdappId.split('.').join('/') + '/MainActivity.java',
            projectPath + '/android/app/src/main/java/' + currentAppIdappId.split('.').join('/') + '/MainApplication.java',
        ];
        ...
        const newPath = srcPath + '/' + newAppIdappId.split('.').join('/') + '/';
